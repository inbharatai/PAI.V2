...
**Root cause (§6 theory, later refuted by §7):** on this stack (Tauri 2 + wry + WebView2, Windows 11), a WebviewWindow created from Rust — regardless of thread — does not initialize its content process. The frontend's JS `new WebviewWindow('browser-workspace', { url: 'about:blank', … })` (the BrowserWorkspace UI path, live-verified since defect #18) was assumed to work every time.

**Interim fix (shipped as PR #44):** stop fighting the runtime — the backend asks the frontend to create the window. `ensure_session` emits `unoone:ensure-browser-workspace`; a listener in App.tsx calls `ensureBrowserWorkspaceWindow()` (`lib/browserWorkspaceWindow.ts`), which runs the same construction (same label/geometry/`about:blank`, reuses an existing window, `tauri://created`/`tauri://error` resolution, focus). The backend then polls `wait_for_window_ready` (500 ms eval probe, 200 ms interval, 15 s cap) and binds the session only once the webview actually answers. If the UI never responds, the failure surfaces honestly to the model.

Desktop suite green (138/138), frontend `oxlint` + `vite build` green.

## 7. Follow-up defect #41: PR #44 live-refuted — the JS path was equally dead; the real root cause is defect #21's per-window browser args

**Live-caught 2026-09-15 (third round, user-reported then reproduced):** the user surfaced `Command plugin:window|set_focus not allowed by ACL` from the app. Forensics on the live app (second-by-second EnumWindows + CDP `/json` sweeps while driving the user path through Playwright CDP):

- Start Session → the **reuse branch** ran (backend `get_all_webviews` still listed a `browser-workspace` entry), UI flipped to "Session Active", and the awaited `existing.setFocus()` threw the ACL rejection into the banner.
- Through **75 s of 2-second sweeps: no `Browser Workspace` OS window ever existed** (deep EnumWindows with no visibility/title filter: only `UnoOne Power`, siw helper, PseudoConsole, IME windows) and **no new CDP target ever appeared**. The "existing" entry was a dead shell: registered in the Tauri runtime, no window, no content process.
- So the §6 theory (JS path good, Rust path bad) was wrong: the JS `new WebviewWindow` path had been **equally dead from birth** — its tauri://created fired, the shell registered, the content never started. What defect #18-era testing (2026-09-12/13) verified worked, stopped working after 2026-09-14.

**Real root cause:** the defect #21 fix added `"additionalBrowserArgs": "--autoplay-policy=no-user-gesture-required"` to the **main window** config. Tauri/wry creates a **separate WebView2 environment per webview**, and the JS `WindowOptions` API exposes no `additionalBrowserArgs` (per Tauri docs, webviews with different `additionalBrowserArgs` "must have different data directories"). Every runtime-created window therefore resolved wry's **default** browser arguments — a conflicting environment on the **same** user-data folder (`%LOCALAPPDATA%\com.unoone.power\EBWebView`), so its WebView2 content never started. Wire evidence: the live browser process (single, pid 132280) ran with `--autoplay-policy=no-user-gesture-required --remote-debugging-port=9222` and **not** wry's defaults — the mismatch is the whole story, and it explains all three creation paths (Rust spawn_blocking, Rust main-thread, JS frontend) failing identically, and the browser window last working 2026-09-13 (the day before b37bff9).

**Secondary defect (the user-reported ACL error):** `capabilities/default.json` never granted `core:window:allow-set-focus`; the BrowserWorkspace reuse branch `await`s `setFocus()`, so the rejection aborted the flow and surfaced as the banner error.

**Fix (four parts):**

1. **Root cause:** removed `additionalBrowserArgs` from tauri.conf.json; `main.rs` now sets `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` (appending, not replacing, any pre-existing value — live-test CDP flags keep working) **before** the first webview exists. The WebView2 loader appends that variable to **every** environment it creates, so the main window and every runtime-created window get an identical argument set, share one browser process, and defect #21's autoplay policy keeps holding everywhere (now including the browser workspace, which the per-window form never covered).
2. **ACL:** `capabilities/default.json` grants `core:window:allow-set-focus`.
3. **Frontend:** `BrowserWorkspace` never lets `setFocus` abort session start (fire-and-forget with catch, both the reuse and created branches) — focus is a defect #18 nicety, never a gate.
